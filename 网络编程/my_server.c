...
find_name(const char *name)
{
    ...
    {
        if(strcpy(users[i].username,name)==0)
        {
            return i;
        }
    }
...
void
send_data(int conn_fd,const char *string)
{
    if(send(conn_fd,string,strlen(string),0)<0)
    {
        my_err("send",__LINE__);     //my_recv.h中
    }
}
